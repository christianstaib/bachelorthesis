...
Hierfür geben wir eine neue Charakterisierung des Contracted- und Hub-Graphen an und präsentieren anschließend PEOPLE (\textbf{P}r\textbf{E}determined \textbf{O}rder, \textbf{P}runed \textbf{L}ab\textbf{e}l), eine Methode, mit der hierarchische Hub-Graphen mit einer vorgegebenen vertex-to-Level-Funktion erstellt werden können, ohne dass eine Knoten-Kontraktion erforderlich ist.
...
\todo{cite paper sublinear search spaces}
...
\autoref{ch::fig::upward_graph} entspricht somit gleichzeitig dem Downward-Graph des Beispielgraphens.
...
Wir fügen $(t, h)$ Kanten ein, sobald wir \emph{einen} optimalen $t$-$h$-Pfad gefunden haben, auf dem $h$ das größte und $t$ das zweitgrößte Level hat.
...
Ist dies erfüllt, so hatt die Wurzel $t$ auf dem $t$-$h$-Pfad nicht das zweitgrößte Level und damit kann $h$ nicht der Kopf einer Upward-Kante sein.
...
Wird als Graph eine Datenstruktur verwendet, welche die Nachbarn nicht in einer definierten Ordnung ausgibt (etwa eine HashMap), ist dies nicht garantiert.
...
Da er für eine vorgebene vertex-to-level-Funktion geprunte Label liefert, wird dieser Algorithmus PEOPLE, für \textbf{P}r\textbf{E}determined \textbf{O}rder, \textbf{P}runed \textbf{L}ab\textbf{e}l, genannt.
...
Um tatsächlich minimale Label zu erzeugen musst der Algorithmus \ref{alg:people:people} verändert werden:
Kann ein Knoten $v$ über $u$ erreicht werden, aber die Kosten hierfür sind gleichgroß wie bisher, so wird $u$ trotzem als Vorgänger von $v$ gesetzt, wenn ${mop}(u) > {mop}(v)$.

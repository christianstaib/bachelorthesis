...
Dabei kam ein für diese Arbeit entwickeltes Programm zum Einsatz.
...
\begin{table}[h!]
  ...
  \end{tabular}
  \caption{CH Graphen-Kontraktion, 100K runs}
...
\begin{table}[h!]
  ...
  \begin{tabular}{
    ...
    \bottomrule
  \end{tabular}
  \caption{HL Graphen-Kontraktion, 10M runs}
...
Für jeden der Graphen wurde ein Hitting-Set über 100k erstellt, die Knoten die nicht im Hitting-Set waren wurden danach sortiert, wie oft sie insgesammt getroffen wurden.
Dies wurde als level-to-vertex Funktion genutzt, wobei die am wenigst getroffen Knoten zuerst kontrahiert wurden.
Es wurde von vorne herein nur Top-Down Kontraktion erprobt, da Bottom-Up zu teuer erscheint.

\subsubsection{Triviale Heuristik}

Das Kontrahieren mit trivialer Heuristik funktioniert auf den Graphen nicht, da zu viele Kanten eingefügt wurden und die erstellung OOM ging.

OOM nach 146, 112, 1655 Knoten
...
\begin{figure}[p]% Die Daten in den .csv Dateien sind gelippt auf 100. pgf hat Probleme mit zu großen Zahlen.
  ...
  \begin{subfigure}[b]{0.5\textwidth}%
    ...
    \caption{pata}
  \end{subfigure}
  \caption{Anzahl der Kanten während der Graphen-Kontraktion mit trivialer Heuristik}
...
Für die obere Schranke durch den vereinfachten Graph wurden die zuvor erstellen Hub-Graphen der Triangulierungen verwendet.



\todo{Wie performant ist das?}

\todo{Wie weit gekommen, HL alleine, HL \& Hubs}
...
\begin{table}[h!]
  ...
  \begin{tabular}{ %VERGLEICH
    ...
    \midrule
    aegaeis-graph      & 32m                           & \B 1h 26m               \\
    aegaeis-visibility & \B 13h 28m                    & 5h 57m                  \\
    medi-graph         & 57 m                          & \B 3h 11m               \\
    medi-visibility    & \B 1d 19h 10m                 & 21h 57m                 \\
    pata-graph         & 7h \phantom{0} 4m             & \B 1d \phantom{0}4h 26m \\
    pata-visibility    & \B 2d 10h 52m                 & 1d 20h 17m              \\  \bottomrule
  \end{tabular}
  \caption{HL  merged}
  \label{table:ergebnisse:vergleich_was_schneller}
\end{table}

Wie in \autoref{subsection:people:label_size} erwähnt können mit PEOPLE erzeugte Label kleiner sein, als durch Merging erzeugte, was auch hier der Fall ist.
Der Unterschied ist hierbei ein Relation zur Label-Größe nicht besonders groß, aber trotzdem wahrnehmbar.
...
Diese Methode wurde verwendet, um verschiedene Funktionen zu testen, die Ergebnisse sind in \autoref{table:ergebnisse:vtl_vergleich} aufgeführt.
